...
export default function Features() {
  return (
    <section id="features" className="py-[100px] bg-white/[0.01]">
      <div className="max-w-[1100px] mx-auto px-6">
        <div className="text-center mb-14">
          <span className="inline-block text-[12px] font-semibold tracking-[0.12em] uppercase text-violet-400 mb-3.5">Features</span>
          <h2 className="font-sora font-extrabold text-zinc-50 leading-[1.15]" style={{ fontSize: "clamp(30px,4vw,42px)" }}>
            Everything you need to land the job
          </h2>
          <p className="text-base text-zinc-500 mt-3 leading-[1.6] max-w-[500px] mx-auto">Built for serious job seekers who want real results, not just practice.</p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {FEATURES.map(({ icon: Icon, title, desc }) => (
            <div key={title} className="rounded-[20px] p-7 border border-white/[0.08] bg-white/[0.03] transition-all duration-300 hover:border-violet-400/25 hover:bg-violet-400/[0.04] hover:-translate-y-[3px]">
              <div className="w-12 h-12 rounded-[14px] flex items-center justify-center text-violet-400 border border-violet-400/20 mb-[18px]" style={{ background: "rgba(167,139,250,0.12)" }}>
                <Icon size={22} />
              </div>
              <h3 className="font-sora font-bold text-[18px] text-zinc-50 mb-2">{title}</h3>
